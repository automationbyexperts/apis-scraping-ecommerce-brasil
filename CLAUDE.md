...
Brazilian Portuguese translation of `ecommerce-scraping-apis`, published as `github.com/automationbyexperts/apis-scraping-ecommerce-brasil`
(local folder `all_public_actors/APIFY_GITHUB/apis-scraping-ecommerce-brasil`). Same affiliate model: every Apify link
...
- A regional group sits **first** in `groups` (`brasil`: Mercado Livre, Shopee e lojas do Brasil), so a local tool is
